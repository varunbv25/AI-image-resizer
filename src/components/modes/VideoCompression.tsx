...
import type { VideoCompressionSettings, VideoProcessingStatus, VideoMetadata } from '@/types';
...
export function VideoCompression({ onBack, onEditAgain, preUploadedFiles }: VideoCompressionProps) {
  ...
  // Update target size when video is uploaded or resolution changes (single mode)
  useEffect(() => {
    if (!isBatchMode && uploadedVideo) {
  ...
  }, [uploadedVideo, resolution, isBatchMode]);
  ...
  const handleBatchVideoUpload = async (files: File[]) => {
    ...
    try {
      ...
      const items: BatchVideoItem[] = results.map((result) => {
        ...
        const maxPossibleSize = originalSizeMB - 0.01;
  ...
  const handleCompress = async () => {
    ...
    } else if (!isBatchMode && uploadedVideo) {
  ...
  const handleDownload = async () => {
    if (isBatchMode && selectedVideo?.processedVideo) {
      try {
    ...
    } else if (!isBatchMode && processedVideo) {
      try {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50 py-4 px-4">
        <div className="max-w-7xl mx-auto">
              ...
              <CardContent className="pt-6">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-red-700 text-sm mt-1">{batchUploadError}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          {videos.length === 0 && !isBatchUploading && !preUploadedFiles ? (
            <VideoUploader
